...
test("buildPolyHyperGraphFromRegions does not mix free and obstacle ports at the same point", () => {
  const graph = buildPolyHyperGraphFromRegions({
    regions: [
      [
        { x: 0, y: 0 },
  ...
  const mixedFreeObstacleDuplicateGroups = duplicatePortGroups.filter(
    (ports) =>
      ports?.some((port) => port.portId.includes("::obstacle")) &&
      ports.some((port) => !port.portId.includes("::obstacle")),
  )

  expect(sharedObstacleEdgePorts).toHaveLength(2)
  expect(sharedObstacleBoundaryPorts).toHaveLength(2)
  expect(mixedFreeObstacleDuplicateGroups).toEqual([])
})
